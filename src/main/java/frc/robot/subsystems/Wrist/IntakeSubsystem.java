...
public class IntakeSubsystem extends SubsystemBase {
    ...
    public IntakeSubsystem() {
        intakeMotor.configure(
                IntakeConstants.INTAKE_MOTOR_CONFIG, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
    }

    /** starts the motor intaking and then sets it to stop when interupted */
    public Command intake() {
        return startEnd(() -> intakeMotor.set(IntakeConstants.CORAL_INTAKE_SPEED), () -> intakeMotor.set(0));
    }

    /** starts the motor ejecting and then sets it to stop when interupted */
    public Command eject() {
        return startEnd(() -> intakeMotor.set(IntakeConstants.CORAL_EJECT_SPEED), () -> intakeMotor.set(0));
    }

    /** sets the intake motor to start intaking */
    public Command setIntaking() {
        return runOnce(() -> intakeMotor.set(IntakeConstants.CORAL_INTAKE_SPEED));
    }

    /** sets the intake motor to start ejecting */
    public Command setEjecting() {
        return runOnce(() -> intakeMotor.set(IntakeConstants.CORAL_EJECT_SPEED));
    }
