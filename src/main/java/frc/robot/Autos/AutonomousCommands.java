...
public final class AutonomousCommands {
    ...
    public static Command scoreNearestL3(
        ...
        return navigateToNearestScoringPose(drive)
                ...
                .until(elevator.atTargetHeight.and(
                        wrist.atTargetAngle)) // Waits until the wrist and elevator are at the required pos
                .andThen(
    ...
    public static Command scoreNearestL4(
            DriveSubsystem drive, ElevatorSubsystem elevator, WristSubsystem wrist, CoralSubsystem intake) {
        return navigateToNearestScoringPose(drive)
                .andThen(elevator.holdHeight(ElevatorConstants.LEVEL_4_TARGET_HEIGHT) // Moves elevator to L4 Pos
                        .alongWith(wrist.holdAngle(WristConstants.LEVEL_4_POSITION))) // Moves wrist to level 4 angle
                .until(elevator.atTargetHeight.and(
                        wrist.atTargetAngle)) // Waits until the wrist and elevator are at the required pos
                .andThen(
                        intake.eject() // ejects coral
